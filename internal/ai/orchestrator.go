...
func (o *AIOrchestrator) GetTradingDecision(tokenAddress string, marketData map[string]interface{}) *TradingDecision {
        ...
        if gd, ok := decisions["gemini"]; ok && gd.Reasoning == "all_keys_rate_limited" {
                pool := o.gemini.KeyPool()
                log.Printf("🔄 Gemini ALL %d key(s) rate-limited — falling back to Groq + OpenRouter + Huangfing", pool.Size())
        }
...
func (o *AIOrchestrator) consolidate(tokenAddress string, decisions map[string]*TradingDecision) *TradingDecision {
        ...
        for provider, decision := range decisions {
                ...
                // Redistribute Gemini weight to Groq, OpenRouter and Huangfing when all keys exhausted
                if geminiAllLimited && (provider == "groq" || provider == "openrouter" || provider == "huangfing") {
                        extra := o.weights["gemini"] / 3.0
                        weight += extra
                }
        ...
        if geminiAllLimited {
                reasoning = "weighted majority — Gemini all keys limited, Groq+OpenRouter+Huangfing boosted"
        }
