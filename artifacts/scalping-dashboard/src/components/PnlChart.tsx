...
export function PnlChart() {
  const { data: raw } = useGetTrades(
    { filter: "today", limit: 500 },
    { query: { refetchInterval: 15000, queryKey: getGetTradesQueryKey({ filter: "today", limit: 500 }) } }
  );
  ...
  const chartData: ChartPoint[] = sorted.map((t) => {
    cum += t.profitEth;
    return {
      time: new Date(t.exitTime).toLocaleTimeString("id-ID", {
        hour: "2-digit",
        minute: "2-digit",
      }),
    ...
    <div className="bg-card border border-border rounded-lg p-4 space-y-3">
      ...
      {chartData.length === 0 ? (
        <div className="h-[140px] flex items-center justify-center text-muted-foreground text-[11px] font-mono">
          No trades yet today — waiting for first trade...
        </div>
      ) : (
