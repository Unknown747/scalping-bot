...
func handleCommand(action string, params map[string]interface{}) {
        ...
        case "start":
                ...
                if !botState.Running {
                        ...
                        botState.mu.Unlock()
                        go runBot()
                        broadcast("log", map[string]interface{}{"message": "✅ Bot started", "type": "success"})
                } else {
                        botState.mu.Unlock()
                }
...
func executeTrade(token data.TokenData, decision *ai.TradingDecision) {
        ...
        broadcast("log", map[string]interface{}{
                "message": fmt.Sprintf("📊 %s signal on %s | Confidence: %.0f%% | $%.2f",
                        decision.Action, token.Symbol, decision.Confidence, posSize),
                "type": "info",
        })
        ...
        pnl := 0.0
        if win {
                pnl = posSize * (cfg.ScalpingStrategies().MomentumTP / 100.0)
        } else {
                pnl = -posSize * (cfg.ScalpingStrategies().MomentumSL / 100.0)
        }
        ...
        if win {
                ...
                botState.mu.Unlock()
                broadcast("log", map[string]interface{}{
                        "message": fmt.Sprintf("✅ TP HIT: %s +$%.4f", token.Symbol, pnl),
                        "type":    "success",
                })
        } else {
                ...
                botState.mu.Unlock()
                broadcast("log", map[string]interface{}{
                        "message": fmt.Sprintf("❌ SL HIT: %s -$%.4f", token.Symbol, -pnl),
                        "type":    "error",
                })
...
func sendInitialState(conn *websocket.Conn) {
        ...
        msg := WSMessage{
                ...
                Data: map[string]interface{}{
                        "stats":   stats,
                        "running": running,
                        "log":     map[string]interface{}{"message": fmt.Sprintf("🚀 Connected to %s v%s", cfg.Bot.Name, cfg.Bot.Version), "type": "info"},
                },
        }
